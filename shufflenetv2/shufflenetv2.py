...
class ShuffleNetV2(nn.Module):
    ...
    def __init__(self, downsample_block, basic_block, width_mult=1, num_classes=10, small_inputs=True):
        ...
        self.conv1 = nn.Conv2d(24, self.inplanes, kernel_size=3, stride=stride, padding=1, bias=False)
        ...
        self.pre_downsample = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=24, kernel_size=5, padding=2, stride=2),
    ...
    def forward(self, x):
        ...
        print("1 out.size()=",out.size())
        if not self.small_inputs: # NOTE: Undo MaxPool for CIFAR10
            out = F.max_pool2d(out, 3, stride=2, padding=1)
        out = self.layer1(out)
        print("2 out.size()=",out.size())
        out = self.layer2(out)
        print("3 out.size()=",out.size())
        out = self.layer3(out)
        # print("4 out.size()=",out.size())
        out = self.relu(self.bn2(self.conv2(out)))
        # print("5 out.size()=",out.size())
        out = self.avgpool(out)
        # print("6 out.size()=",out.size())
        out = out.view(out.size(0), -1)
        # print("7 out.size()=",out.size())
        out = self.fc(out)
        # print("8 out.size()=",out.size())
        return out
...
if __name__ == '__main__':
    ...
    print(model.layer1)
    print(get_n_params(model))#打印参数数量
    x = torch.randn(16, 3, 32, 32)
    out = model(x)
    # out = model.relu(model.bn1(model.conv1(x)))
    # print("0 out.size():",out.size())
    # if not model.small_inputs: # NOTE: Undo MaxPool for CIFAR10
    #     out = F.max_pool2d(out, 3, stride=2, padding=1)
